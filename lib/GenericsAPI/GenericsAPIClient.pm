...
=head2 export_corr_matrix_excel

  $result = $obj->export_corr_matrix_excel($params)
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function export_corr_matrix_excel (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to export_corr_matrix_excel:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'export_corr_matrix_excel');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "GenericsAPI.export_corr_matrix_excel",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method export_corr_matrix_excel",
					    status_line => $self->{client}->status_line,
					    method_name => 'export_corr_matrix_excel',
				       );
    }
...
=head2 compute_correlation_across_matrices

  $returnVal = $obj->compute_correlation_across_matrices($params)
...
<pre>
$params is a GenericsAPI.CompCorrMetriceParams
$returnVal is a GenericsAPI.CompCorrOutput
CompCorrMetriceParams is a reference to a hash where the following keys are defined:
	matrix_ref_1 has a value which is a GenericsAPI.obj_ref
	matrix_ref_2 has a value which is a GenericsAPI.obj_ref
	workspace_name has a value which is a GenericsAPI.workspace_name
	corr_matrix_name has a value which is a string
	dimension has a value which is a string
	method has a value which is a string
...
=cut
 ...
 sub compute_correlation_across_matrices
{
    my($self, @args) = @_;
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function compute_correlation_across_matrices (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to compute_correlation_across_matrices:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'compute_correlation_across_matrices');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "GenericsAPI.compute_correlation_across_matrices",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method compute_correlation_across_matrices",
					    status_line => $self->{client}->status_line,
					    method_name => 'compute_correlation_across_matrices',
				       );
    }
